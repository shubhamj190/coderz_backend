...
class StudentCreateSerializer(serializers.ModelSerializer):
    ...
    # Grade is required but DivisionId is only used for grouping logic, not stored in UserDetails
    GradeId = serializers.PrimaryKeyRelatedField(queryset=Grade.objects.all(), required=True)
    DivisionId = serializers.PrimaryKeyRelatedField(queryset=Division.objects.all(), required=True, write_only=True)
    ...
    class Meta:
        ...
        fields = [
            ...
            "last_name",
            "gender",
            "DivisionId",  # This will be used only in serializer, not stored in UserDetails
        ]
    ...
    def create(self, validated_data):
        ...
        last_name = validated_data.pop("last_name")
        gender = validated_data.pop("gender")
        GradeId = validated_data.pop("GradeId")  # Model instance
        DivisionId = validated_data.pop("DivisionId")  # Model instance (Not in UserDetails)
        ...
        with transaction.atomic():
            ...
            # Create the Student record linking to the user (DivisionId is NOT stored here)
            student = UserDetails.objects.create(
                ...
                GradeId=GradeId,  # ForeignKey field in UserDetails
            ...
            # 🔎 Find GroupMaster based on Grade and Division
            group_name = f"{GradeId.GradeName} - {DivisionId.DivisionName}"
            group = GroupMaster.objects.filter(GroupName=group_name).first()

            if group:
                # 🏫 Get Location (assuming LID=2 for Institution)
